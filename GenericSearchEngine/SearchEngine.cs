...
namespace GenericSearchEngine {
    ...
    /// </summary>
    public abstract class SearchEngine<TBoard, TMove> : SearchEngine 
...
#if MOVEISCLASS
            FindBestMove(board, searchEngineSetting, playerId, isMaximizing, moveList, attackPosInfo, out TMove? move, out int permCount, out long cacheHit, out int maxDepth);
#else
            move = board.CreateEmptyMove();
            FindBestMove(board, searchEngineSetting, playerId, isMaximizing, moveList, attackPosInfo, ref move, out int permCount, out long cacheHit, out int maxDepth);
#endif
