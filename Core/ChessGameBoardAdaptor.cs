...
namespace ChessMaster.Core
{
    ...
    internal sealed class ChessGameBoardAdaptor : IGameBoard<Move>
    {
        ...
        /// </summary>
        public long ZobristKey => ChessBoard.ZobristKey;
        ...
        /// Board clone
        /// </returns>
        public ChessGameBoardAdaptor Clone() => new(ChessBoard.Clone(), Dispatcher);
        ...
        /// true if not a draw
        /// </returns>
        public bool DoMoveNoLog(Move move) => ChessBoard.DoMoveNoLog(move) == ChessBoard.RepeatResult.NoRepeat;

        /// <summary>
        /// Undo the move without log
        /// </summary>
        /// <param name="move"> Move to be undone</param>
        public void UndoMoveNoLog(Move move) => ChessBoard.UndoMoveNoLog(move);
        ...
        public int EvaluateBoard(SearchEngineSetting searchSetting,
             ...
             => ChessBoard.Points(searchSetting,
        ...
        public List<Move> GetMoves(int playerId, out AttackPosInfo attackPosInfo)
            => ChessBoard.EnumMoveList((ChessBoard.PlayerColor)playerId, isMoveListNeeded: true, out attackPosInfo)!;
        ...
        /// Extra info
        /// </returns>
        public int ComputeBoardExtraInfo() => (int)ChessBoard.ComputeBoardExtraInfo(addRepetitionInfo: true);
        ...
        /// </returns>
        public bool IsEnoughPieceForWinning() => ChessBoard.IsEnoughPieceForCheckMate();
        ...
        /// </returns>
        public bool IsWinningPts(int pts) => pts is >= 1000000 and <= 1000099 or >= (-1000099) and <= (-1000000);
                                   ...
                                   out int pts)
        {
            ...
            if (moveList.Count == 0)
            {
                if (ChessBoard.IsCheck((ChessBoard.PlayerColor)moveListPlayerId))
                {
                ...
                {
                    pts = 0; // Draw
                    minMaxInfo.TransTable?.RecordEntry(moveListPlayerId, ChessBoard.ZobristKey, extraInfo: 0, pts, weight: 255);
                }
            }
            else
            {
                ...
                if (!IsEnoughPieceForWinning())
                {
                    ...
                    minMaxInfo.TransTable?.RecordEntry(moveListPlayerId, ChessBoard.ZobristKey, boardExtraInfo, pts, weight: 255);
                }
                else if (minMaxInfo.Depth == 0 || isSearchHasBeenCanceled)
                {
                    pts = minMaxInfo.TransTable?.ProbeEntry(moveListPlayerId,
                                                            ChessBoard.ZobristKey,
                                                            boardExtraInfo,
