...
namespace ChessMaster.Core
{
    ...
    public struct Move
    {
        ...
        public string GetHumanPos()
        {
            ...
            retVal = $"{GetHumanPos(StartPos)}{((Type & MoveType.PieceEaten) == MoveType.PieceEaten ? "x" : "-")}{GetHumanPos(EndPos)}";
            switch (Type & MoveType.MoveTypeMask)
            {
            ...
            if ((Type & MoveType.MoveFromBook) == MoveType.MoveFromBook)
            {
                retVal = $"({retVal})";
            }
            return retVal;
