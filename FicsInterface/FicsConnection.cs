...
namespace ChessMaster.FicsInterface
{
    /// <summary>
    ...
    public class FicsConnection : IDisposable
    {
        ...
        private class AutomatonState : IDisposable
        {
            ...
            public void SetCommand(CmdExecuting cmd, FicsGameIntf? gameIntf)
            {
                ...
                lock (this)
                {
                    if (CmdExecuting is not CmdExecuting.None and not CmdExecuting.PreLogin)
                    {
        ...
        public FicsConnection(ChessBoardControl mainCtl, string hostname, int port, bool debugTrace)
        {
            ...
            OriginalSettings = new Dictionary<string, string>(512, StringComparer.OrdinalIgnoreCase);
        ...
        /// Original settings
        /// </summary>
        public Dictionary<string, string> OriginalSettings { get; private set; }
        ...
        {
            if (!addToSet || OriginalSettings.ContainsKey(settingName))
            {
        ...
        public void RestoreOldSetting()
        {
            ...
            {
                oldValue = OriginalSettings[strSetting];
                          ...
                          out string? errTxt)
        {
            ...
            if (m_state.CmdSignal!.WaitOne(timeOut * 1000))
            {
                if (m_state.LastCmdError == null)
                {
                    ...
                    if (GetVariableList(timeOut * 1000) > 20)
                    { // At least 20 variables are defined
                        SetQuietMode();
                        retVal = true;
                    }
                    else
        ...
        public int GetVariableList(int timeOut)
        {
            ...
            if (m_state.CmdSignal!.WaitOne(timeOut * 1000))
            {
                OriginalSettings = new Dictionary<string, string>(m_state.VariableList, StringComparer.OrdinalIgnoreCase);
            }
            else
            {
                m_state.ResetCommand();
            }
            return OriginalSettings.Count;
        }
        ...
        private bool ProcessSingleMove(string line)
        {
            ...
            if (errTxt != null)
            {
                m_mainCtl.Dispatcher.Invoke(() => { m_mainCtl.ShowError($"Error decoding a move - {errTxt}\r\n({line})"); });
                retVal = true;
            }
        ...
        private void GettingVariableListValue(string line)
        {
            ...
            if (!string.IsNullOrEmpty(line))
            {
                ...
                { // Guest doesn't receive the Formula setting
                    m_state!.ResetCommand();
                }
            }
        }
