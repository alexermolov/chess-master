...
using ChessMaster.FicsInterface;
using ChessMaster.Helpers;
...
namespace ChessMaster
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private FicsConnection? m_ficsConnection;
        /// <summary>Setting to connect to the FICS server</summary>
        private readonly FicsConnectionSetting m_ficsConnectionSetting;
        ...
        public MainWindow()
        {
            ...
            MoveAlternativesGrid.ChessControl = m_chessCtl;
            ...
            MoveAlternativesGrid.NewMoveSelected += MoveAlternativesGrid_NewMoveSelected;
            ...
            Background = (Brush)bc.ConvertFrom("#212121");
        }

        private void MoveAlternativesGrid_NewMoveSelected(object? sender, NewGameMoveSelectedEventArg e)
        {
            ChessBoard.GameResult result;

            if (PlayingMode == MainPlayingMode.PlayerAgainstPlayer)
            {
                GameSelected(e.NewGameIndex);

                result = m_chessCtl.SelectMove(e.NewStepIndex, out bool succeed);
        ...
        private void GameSelected(int gameIndex)
        {
            string? game;
            PgnParser parser;

            game = GameService.Games[gameIndex].GameToText();

            if (game != null)
            {
                parser = new PgnParser(false);
                parser.InitFromString(game);
                var parsed = parser.ParseSingle(false, false, out int skip, out int truncated, out var pgnGame, out string? errTxt);
                m_chessCtl
                    .CreateGameFromMove(pgnGame.StartingChessBoard, pgnGame.MoveExtList, pgnGame.StartingColor,
                        pgnGame.WhitePlayerName ?? "", pgnGame.BlackPlayerName ?? "", pgnGame.WhiteType, pgnGame.BlackType, pgnGame.WhiteSpan, pgnGame.BlackSpan);
        ...
        public bool IsComputerMustPlay
        {
            get
            {
                ...
                if (retVal)
                {
                    ...
                    retVal = moveResult is ChessBoard.GameResult.OnGoing or ChessBoard.GameResult.Check;
                }
                return retVal;
            }
        }
        ...
        private bool CheckIfDirty()
        {
            ...
            {
                switch (MessageBox.Show("Board has been changed. Do you want to save it?", "ChessMaster", MessageBoxButton.YesNoCancel))
                {
        ...
        private void TestBoardEvaluation_PlayNextMove(BoardEvaluationStat stat, object? move)
        {
            ...
            if (result is ChessBoard.GameResult.OnGoing or ChessBoard.GameResult.Check)
            {
                if (!m_chessCtl.FindBestMove(null, (x, y) => TestBoardEvaluation_PlayNextMove(x, y), stat))
                {
                    throw new ApplicationException("How did we get here?");
        ...
        private void ObserveFICSGame()
        {
            ...
            if (!CheckIfDirty())
            {
                ...
                if (findGameFrm.ShowDialog() == true)
                {
                    ...
                    m_toolbar.labelWhitePlayerName.ToolTip = $"Rating = {FicsGame.GetHumanRating(game.WhiteRating)}";
                    m_toolbar.labelBlackPlayerName.Content = $"({game.BlackPlayerName}) :";
                    m_toolbar.labelBlackPlayerName.ToolTip = $"Rating = {FicsGame.GetHumanRating(game.BlackRating)}";
        ...
        private void ObserveFinished(FicsGameIntf? ficsGameIntf, FicsInterface.TerminationCode terminationCode, string msg)
        {
            ...
            {
                Dispatcher.Invoke(() => { ObserveFinished(ficsGameIntf, terminationCode, msg); });
            }
        }
        ...
        {
            mnuOptionsReversedBoard.IsChecked = !mnuOptionsReversedBoard.IsChecked;
            m_chessCtl.IsBoardReversed = mnuOptionsReversedBoard.IsChecked;
        }
        ...
        /// <param name="sender"> Sender object</param>
        /// <param name="e">      Event handler</param>
        private void ChessCtl_MoveSelected(object? sender, ChessBoardControl.MoveSelectedEventArgs e) => m_chessCtl.DoUserMove(e.Move);
